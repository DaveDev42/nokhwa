...
- `Camera<Mjpeg>::new(index, requested)` is used by the `single` subcommand because `RequestedCliFormat::make_requested` already builds a `RequestedFormat`; `camera.frame_typed()` then yields a `Frame<Mjpeg>` and `frame.into_rgb().materialize()` decodes to an `RgbImage`.
