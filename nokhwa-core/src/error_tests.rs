...
// Every `Display`-format variant is pinned verbatim below. The earlier
// `contains(...)` keyword tests have been removed — they were strictly
// weaker than the `_exact_format` pins (a wording change that broke the
// Display string but kept the substrings would have passed the contains
// checks while regressing the user-visible output and any log-scraper
// that keys on the exact prefix). Guards `nokhwa-core/src/error.rs:25-74`.
#[test]
