...
- **`CHANGELOG.md` is generated by release-please** from squash-commit subjects. Do NOT hand-edit the `## Unreleased` section to accumulate prose — every commit subject already lands in the next versioned section automatically, grouped by conventional-commit prefix (`feat:` → Features, `fix:` → Bug Fixes, `perf:` → Performance, `refactor:` → Refactoring, `chore:` / `ci:` → Infrastructure, `docs:` → Documentation, `test:` → Testing — per `release-please-config.json`). Detailed rationale lives in PR descriptions, commit message bodies, and `TODO.md`'s "Shipped recently" log. Only hand-edit `CHANGELOG.md` when fixing historical sections — never to add unshipped notes.
