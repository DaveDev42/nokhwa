...
- [ ] Re-implement GStreamer backend (cross-platform, previously ~839 lines). Multi-session rollout in progress:
  ...
  - [ ] **Session 3**: Controls via `gst-properties` on the source element (best-effort; GStreamer does not expose a canonical `KnownCameraControl` → property map the way MSMF/V4L2 do).
  - [ ] **Session 4** (optional): `nokhwa::open()` dispatch integration so `open(CameraIndex::Index(0), OpenRequest::any())` picks a GStreamer device when the native backends are unavailable.
