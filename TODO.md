...
- [ ] Re-implement GStreamer backend (cross-platform, previously ~839 lines). Multi-session rollout in progress:
  ...
  - [ ] **Session 4** (optional): `nokhwa::open()` dispatch integration so `open(CameraIndex::Index(0), OpenRequest::any())` picks a GStreamer device when the native backends are unavailable, plus URL-like `CameraIndex::String("rtsp://...")` routing.
  - [ ] **Session 5 (new) — RTSP/URL path.** Detect `CameraIndex::String` matching `rtsp://` / `http://` / `file://` and build `rtspsrc/souphttpsrc/filesrc ! decodebin ! videoconvert ! appsink` instead of the `Device::create_element` path. Required before evaluating OpenCV backend removal, since OpenCV's unique value today is IP/RTSP cameras via `VideoCapture::from_file`.
